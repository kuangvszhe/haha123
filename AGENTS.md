...
上下文窗口是有限的资源。以下规则自动执行：

| 当前上下文使用率 | 自动执行 |
|---|---|
| < 50% | 正常流程 |
| 50-65% | 优先用 `edit_file` 代替 `write_file`（增量编辑节省token） |
| 65-80% | 回答尽量简洁；删已处理的 tool result |
| 80-90% | 触发 `auto-compact` 压缩 |
| > 90% | 强制 `auto-compact` + 截断早期消息 |

### 节省 token 的具体策略
1. 用 `edit_file`（精确替换）而非 `write_file`（全文重写）—— **token 差 10x**
2. 不用 `cat` 读大文件，用 `read_file(limit=N)` 分段读
3. 已确认的信息不重复展示
4. 成功的 bash 命令不输出 stdout（只看 exit code）
5. 失败的 bash 命令才展示输出（诊断才有价值）
